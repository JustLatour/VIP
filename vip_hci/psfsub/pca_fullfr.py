...
def pca(*all_args: List, **all_kwargs: dict):
    ...
    ncomp : int, float, tuple of int/None, or list, optional
        ...
        * ADI (``cube`` is a 3d array): if an int is provided, ``ncomp`` is the
        ...
        ``source_xy`` is not None, then the S/Ns (mean value in a 1xFWHM circular
        aperture) of the given (X,Y) coordinates are computed.
        ...
        for ch in range(nch):
            ...
            if z_r < (nz -1):
                ...
                Ind_RDI_Left = cube_detect_badfr_correlation(resc_cube_ref_rdi, 
                                    RefFrame, verbose = False, crop_size = x_r, 
            ...
            sdi_cube_left = np.concatenate(
                tuple(algo_params.cube[i, :, :, :] for i in Ind_Channels_Left))
            z_sdi = sdi_cube_left.shape[0]
            rescaling_sdi_coeff = np.array([[
                algo_params.scale_list[i]] * nz for i in Ind_Channels_Left]).flatten()
            ...
            Ind_SDI_Left = cube_detect_badfr_correlation(resc_cube_ref_sdi, RefFrame, 
                verbose = False, crop_size = nz, percentile = Percentile_sdi, 
                plot = False)[0]
    ...
    elif algo_params.scale_list is not None:
    ...
    # ADI and ADI+RDI (3D or 4D)
        ...
        if (
            algo_params.cube_ref is not None or algo_params.source_xy is None
        ) and algo_params.full_output:
            # incremental PCA
    ...
    **rot_options,
    ...
    if np.isscalar(ncomp):
        ...
        for frame in range(z):
            ...
            print(cube_ref_adi.shape, cube_ref_rdi.shape, cube_ref_sdi.shape)
            res_result = _project_subtract(
                cube,
                cube_ref_arsdi,
                ncomp,
...
def _project_subtract(
    ...
    if isinstance(ncomp, (int, np.int_)):
        ...
        svdecomp = SVDecomposer(
            cube, mode="fullfr", svd_mode=svd_mode, scaling=scaling, verbose=verbose
        )
