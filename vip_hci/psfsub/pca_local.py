...
def pca_annular_stim(*all_args: List, **all_kwargs: dict):
    """PCA model PSF subtraction for ADI, ADI+RDI or ADI+mSDI (IFS) data.
...
def do_pca_patch_stim(
    ...
    if isinstance(ncomp, list):
        residuals = []
        reconstructed = np.zeros_like(curr_frame_emp)
        prev = 0
        #Assumes that ncomp values are in increasing order
        for nn, npc_tmp in enumerate(ncomp):
            transformed = np.dot(curr_frame_emp, V[prev:npc_tmp].T)
            reconstructed = reconstructed + np.dot(transformed.T, V[prev:npc_tmp])
            residuals.append(curr_frame - reconstructed)
            prev = npc_tmp
    else:
        transformed = np.dot(curr_frame_emp, V.T)
